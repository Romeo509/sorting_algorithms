...
/**
 * int_to_card - turn into integer the card value
 * @head: pointer to the list
 *
 * Return: integer representation
 **/
int int_to_card(deck_node_t *head)
{
	int aux_num, j;
...
void num_sort(deck_node_t **list)
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		{
			aux_num1 = int_to_card(head_tmp1);
			aux_num2 = int_to_card(head_tmp1->next);
			flag = 0;
			head_tmp2 = head_tmp1;
			while (head_tmp2 && head_tmp2->card->kind == k && aux_num1 > aux_num2)
			{
				...
				if (!head_tmp2)
					break;
				aux_num1 = int_to_card(head_tmp2);
				aux_num2 = int_to_card(head_tmp2->next);
			}
			if (flag == 0)
